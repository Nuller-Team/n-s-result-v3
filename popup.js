...
chrome.storage.local.get('reports', function (result) {
    ...
    for (var subject in report) {
        for (let i in report[subject]) {
            ...
            subject_done.innerText = report[subject][i]["done"].replace("%", "");
            ...
            subject_percent_done.style.width = report[subject][i]["done"];
            subject_percent_done.style.height = "100%";
            subject_percent_done.style.borderRadius = "10px";
            subject_percent_done.style.background = report[subject][i]["done"] === "100%" ? "var(--green)" : "var(--red)";
